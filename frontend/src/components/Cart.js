...
const Cart = () => {
  ...
  async function displayRazorpay() {
    ...
    const data = await axios.post('http://localhost:8000/create/orderId', { amount: amnt }).then((t) => t)
    console.log(data)
    const options = {
      ...
      handler: async (response) => {
        ...
        console.log(response);
        try {
          const dt = await axios.post('http://localhost:8000/verifyPayment', response);
          if (dt === 'success') {
